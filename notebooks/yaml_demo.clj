...
(let [_run-at #inst "2024-03-16T03:40:35.357-00:00"]
  (def example-file "test/example.yml"))

^{::clerk/visibility {:code :fold :result :hide}}
(def backlog (yaml/backlog example-file))
...
;; ^{::clerk/visibility {:code :hide :result :hide}}
(clerk/with-viewer mermaid-viewer (mermaid/graph (graph/graph {:include-status? true} backlog)))

;; ^{::clerk/visibility {:code :hide :result :hide}}
(clerk/with-viewer mermaid-viewer (mermaid/graph (graph/simplified {:include-status? true} backlog)))
